...
//! assert_eq!(info.engines, "sherpa-onnx");
...
mod tests {
    ...
    fn engines_field_is_populated() {
        let sherpa = models()
            ...
            .count();
        assert!(sherpa > 1500, "sherpa-drivable: {sherpa}");
        assert_eq!(sherpa, models().len(), "every model should be sherpa-onnx");
    }
    ...
    fn spot_checks() {
        ...
        assert_eq!(m.engines, "sherpa-onnx");
        ...
        assert_eq!(k.engines, "sherpa-onnx");
    }
}
